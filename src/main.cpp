...
uint256 hashGenesisBlock("0x124d4d33e19cc5abd4215dcbc4f9ccdb20380f01fbff465403a2da92a96c16f1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // POPCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "POPCoin Signed Message:\n";
...
{
    // POPCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60 * 60; // Popcoin: 1 Hour
static const int64 nTargetSpacing = 51; // Popcoin: 51 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PopularCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64        BlocksTargetSpacing                        = 51; // 51 seconds
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PopularCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PopularCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Phnom Penh 01/19/2014 In Mean, POPCOIN";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1390880416;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085460295;

        if (fTestNet)
        {
            block.nTime    = 139013150;
        ...
        assert(block.hashMerkleRoot == uint256("0x2eeb503b4344ab293edc4700ab989b44d411fe0d24c199f2d245c2fdcc307b44"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb2, 0xda }; // Popcoin
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("POPCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("POPCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PopularCoinMiner(CWallet *pwallet)
{
    printf("POPCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("popcoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running POPCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("POPCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PopularCoinMiner, pwallet));
}
